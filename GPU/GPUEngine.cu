...
__global__ void comp_keys(address_t* sAddress, uint32_t* lookup32, uint64_t* keys, uint32_t* out, PubKeyData* kernel_targets, int kernel_numTargets) {
    // 設置內核中可見的全局指針
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        d_targets = kernel_targets;
        d_numTargets = kernel_numTargets;
    }
    __syncthreads(); // <--- FIX: __syncthreads
...
GPUEngine::GPUEngine(int gpuId, uint32_t maxFound) {
    ...
    // 初始化 CUDA
    this->rekey = false; // Initialize rekey
    initialised = false;
    cudaError_t err;
...
bool GPUEngine::callKernel() {
    ...
    comp_keys<<<nbThread / NB_TRHEAD_PER_GROUP, NB_TRHEAD_PER_GROUP>>>(
        inputAddress, inputAddressLookUp, inputKey, outputBuffer, d_targets, numTargets);
...
bool GPUEngine::Launch(std::vector<ITEM>& addressFound, bool spinWait) {
    ...
    for (uint32_t i = 0; i < nbFound; i++) {
        ...
        if ((searchMode == SEARCH_MODE_PUBKEY) && (searchType == PUBKEY)) {

            it.foundIndex = *((int*)(&itemPtr[2]));
            it.hash = nullptr; // 公鑰模式沒有 hash
        } else {
...
// 18. 重寫 SetTargetPublicKey 以處理公鑰列表

void GPUEngine::SetTargetPublicKey(const std::vector<PubKeyData>& targets) {
    this->searchMode = SEARCH_MODE_PUBKEY; 

    this->searchType = PUBKEY;

    this->numTargets = targets.size();
    if (this->numTargets == 0) {
        fprintf(stderr, "GPUEngine Error: Target public key list is empty.\n");
        return;
    }

    cudaError_t err = cudaMalloc((void**)&this->d_targets, this->numTargets * sizeof(PubKeyData));
    if (err != cudaSuccess) {
        printf("GPUEngine: CudaMalloc for d_targets failed: %s\n", cudaGetErrorString(err));
        return;
    }

    err = cudaMemcpy(this->d_targets, targets.data(), this->numTargets * sizeof(PubKeyData), cudaMemcpyHostToDevice);
    if (err != cudaSuccess) {
        printf("GPUEngine: CudaMemcpy to d_targets failed: %s\n", cudaGetErrorString(err));
    }
    
    uint32_t searchInfoValue = SEARCH_MODE_PUBKEY;
    err = cudaMemcpyToSymbol(d_searchInfo, &searchInfoValue, sizeof(uint32_t));
    if (err != cudaSuccess) {
        printf("GPUEngine: CudaMemcpyToSymbol d_searchInfo (pubkey mode) failed: %s\n", cudaGetErrorString(err));
    }
}

void GPUEngine::SetSearchType(int searchType) {
    this->searchType = searchType;
    this->searchMode = SEARCH_MODE_ADDRESS; // Set search mode for Launch parsing

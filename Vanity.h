...
// (TH_PARAM, ADDRESS_ITEM, ADDRESS_TABLE_ITEM) 
typedef struct {
	VanitySearch* obj;
	int  threadId;
	bool isRunning;
...
private:
    ...
    void updateFound();
    void getGPUStartingKeys(Int& tRangeStart, int stepSize, int groupSize, int numThreadsGPU, Point* publicKeys, uint64_t Progress);

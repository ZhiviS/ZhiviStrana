...
bool VanitySearch::initAddress(std::string& address, ADDRESS_ITEM* it, int index) {
    ...
    if (isCompressedPubKey || isUncompressedPubKey) {
        if (searchType != -1 && searchType != PUBKEY) {
            fprintf(stdout, "[ERROR] Ignoring public key \"%s\". Cannot mix public key search with address search.\n", address.c_str());
            return false;
        }
        ...
        PubKeyData newTarget;
        memcpy(newTarget.x, x_coord.bits64, sizeof(newTarget.x));

        if (isCompressedPubKey) {
            newTarget.parity = (address.substr(0, 2) == "03");
            ...
            newTarget.parity = p.y.IsOdd();
        }

        this->targetPubKeys.push_back(newTarget);
        return false;
    }
...
void VanitySearch::getGPUStartingKeys(Int& tRangeStart, int stepSize, int groupSize, int nbThread, Point *p, uint64_t Progress) {
...
void VanitySearch::FindKeyGPU(TH_PARAM* ph) {
    ...
    while (!endOfSearch) {
        ...
        Int batch_size;
        batch_size.SetInt32(numThreadsGPU);
        batch_size.Mult(STEP_SIZE);

        Int batch_start_key;
        Int current_offset;
        
        current_offset.SetInt32(idxcount);
        current_offset.Mult(&batch_size);
        
        batch_start_key.Set(&bc->ksStart);
        batch_start_key.Add(&current_offset);

        if (batch_start_key.IsGreaterOrEqual(&bc->ksFinish)) {
            endOfSearch = true;
            delete[] publicKeys;
            printf("\n[+] Search range completed.\n");
            fflush(stdout);
            continue;
        }
        ...
        getGPUStartingKeys(batch_start_key, STEP_SIZE, g.GetGroupSize(), numThreadsGPU, publicKeys, 0);
        ...
        if (ok && !endOfSearch && !Pause) {
            ...
            for (int i = 0; i < (int)found.size(); i++) {
                ...
                Int thread_offset_in_batch;
                thread_offset_in_batch.SetInt32(it.thId);
                thread_offset_in_batch.Mult(STEP_SIZE);
    
                Int privkey;
                privkey.Set(&batch_start_key);
                privkey.Add(&thread_offset_in_batch);
                privkey.Add((uint64_t)it.incr);
                
                if (this->searchType == PUBKEY) {
                     if (it.foundIndex >= 0 && it.foundIndex < (int)this->inputAddresses.size()) {
                        output(this->inputAddresses[it.foundIndex], secp->GetPrivAddress(true, privkey), privkey.GetBase16());
                    } else {
                        fprintf(stderr, "\n[ERROR] Invalid foundIndex %d received from GPU.\n", it.foundIndex);
                    }
                    nbFoundKey++;
                } else {
                    checkAddr(*(address_t*)(it.hash), it.hash, privkey, 0, it.endo, it.mode);
                }
            }
            ...
            keys_done.Mult(&batch_size);
            
            uint64_t keys_n_done = keys_done.ToDouble();
